...
from web_core_reference.configure.excepts import DubbingSrtError, StopTask
from web_core_reference.configure.config import tr, settings, logger, ROOT_DIR
    ...
    def _cleantts(self) -> None:
        ...
        if settings.get('normal_text'):
            if self.language[:2] == 'zh':
                from web_core_reference.util.cn_tn import TextNorm
                normalizer = TextNorm(to_banjiao=True)
            elif self.language[:2] == 'en':
                from web_core_reference.util.en_tn import EnglishNormalizer
