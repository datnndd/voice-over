...
from web_core_reference.configure.config import ROOT_DIR, tr, settings, params, logger
...
def get_camb_role(force=False, raise_exception=False):
    ...
    jsonfile = f'{ROOT_DIR}/web_core_reference/voicejson/camb.json'
    namelist = ["No", "clone"]
...
def get_elevenlabs_role(force=False, raise_exception=False):
    from . import help_misc
    jsonfile = f'{ROOT_DIR}/web_core_reference/voicejson/elevenlabs.json'
    namelist = ["No"]
    if help_misc.vail_file(jsonfile):
        with open(jsonfile, 'r', encoding='utf-8-sig') as f:
            cache = json.loads(f.read())
...
def get_piper_role():
    file_path = f"{ROOT_DIR}/web_core_reference/voicejson/piper.json"
    if Path(file_path).exists():
        rolelist = json.loads(Path(file_path).read_text(encoding='utf-8-sig'))
    else:
        rolelist = {}
        from web_core_reference.translator import LANGNAME_DICT
...
def get_302ai():
    ...
    with open(ROOT_DIR + "/web_core_reference/voicejson/302.json", 'r', encoding='utf-8-sig') as f:
...
def get_doubao2_rolelist(role_name=None, langcode="zh"):
    roledata = json.loads(Path(f'{ROOT_DIR}/web_core_reference/voicejson/doubao2.json').read_text(encoding='utf-8-sig'))
...
def get_edge_rolelist(role_name=None, locale=None):
    from . import help_misc
    voice_list = {}
    voice_file = ROOT_DIR + "/web_core_reference/voicejson/edge_tts.json"
...
def get_azure_rolelist(language=None, role_name=None):
    voice_file = ROOT_DIR + "/web_core_reference/voicejson/azure_voice_list.json"
...
def get_minimaxi_rolelist():
    from . import help_misc
    voice_list = {}
    voice_file = ROOT_DIR + "/web_core_reference/voicejson/minimaxi.json"

    if params.get("minimaxi_apiurl", '') == 'api.minimax.io':
        voice_file = ROOT_DIR + "/web_core_reference/voicejson/minimaxiio.json"
    if help_misc.vail_file(voice_file):
        try:
            with open(voice_file, 'r', encoding='utf-8-sig') as f:
                voice_list = json.loads(f.read())
...
def get_qwen3tts_rolelist():
    voices = json.loads(Path(ROOT_DIR + "/web_core_reference/voicejson/qwen3tts.json").read_text(encoding='utf-8-sig'))
    voices = {"No": "No"} | voices
    return voices
...
def get_supertonic_rolelist():
    voices = json.loads(Path(ROOT_DIR + "/web_core_reference/voicejson/supertonic.json").read_text(encoding='utf-8-sig'))
    voices = {"No": "No"} | voices
    return voices


def get_glmtts_rolelist():
    voices = json.loads(Path(ROOT_DIR + "/web_core_reference/voicejson/glmtts.json").read_text(encoding='utf-8-sig'))
    voices = {"No": "No"} | voices
    return voices
...
def role_menu(tts_type, langcode=None) -> List:
    from web_core_reference import tts
...
def show_refaudio_win():
    raise RuntimeError("Ref audio window is unavailable in headless snapshot")
