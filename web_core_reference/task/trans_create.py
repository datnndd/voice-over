...
from web_core_reference import translator
from web_core_reference.configure.config import ROOT_DIR, tr, app_cfg, settings, logger
from web_core_reference.configure import config
from web_core_reference.recognition import run as run_recogn, is_allow_lang as recogn_allow_lang, FASTER_WHISPER
from web_core_reference.translator import run as run_trans, get_audio_code
from web_core_reference.tts import run as run_tts, EDGE_TTS, AZURE_TTS, SUPPORT_CLONE
from web_core_reference.task.simple_runnable_qt import run_in_threadpool
from web_core_reference.util import tools
from ..configure import contants
from ._base import BaseTask
from web_core_reference.util.help_ffmpeg import get_video_codec
from web_core_reference.task._rate import SpeedRate
...
class TransCreate(BaseTask):
    ...
    def recogn(self) -> None:
        ...
        if self.cfg.remove_noise:
            ...
            if tools.vail_file(_remove_noise_wav):
                ...
                tools.down_file_from_ms(f'{ROOT_DIR}/models/onnx', urls=[
                    'https://modelscope.cn/models/himyworld/web_core_reference/resolve/master/onnx/dpdfnet4.onnx'],
                                        callback=self._process_callback)
                from web_core_reference.process.prepare_audio import remove_noise
                kw = {
        ...
        if self.cfg.fix_punc and self.cfg.detect_language[:2] in ['zh', 'en']:
            tools.down_file_from_ms(f'{ROOT_DIR}/models/puntc', [
                    "https://www.modelscope.cn/models/himyworld/web_core_reference/resolve/master/puntc/model.onnx",
                    "https://www.modelscope.cn/models/himyworld/web_core_reference/resolve/master/puntc/config.yaml",
                    "https://www.modelscope.cn/models/himyworld/web_core_reference/resolve/master/puntc/tokens.json",
            ], callback=self._process_callback)
            from web_core_reference.process.prepare_audio import fix_punc
            # 预先删掉已有的标点
        ...
        try:
            ...
            if speaker_type == 'built':
                tools.down_file_from_ms(f'{ROOT_DIR}/models/onnx', [
                    "https://www.modelscope.cn/models/himyworld/web_core_reference/resolve/master/onnx/seg_model.onnx",
                    "https://www.modelscope.cn/models/himyworld/web_core_reference/resolve/master/onnx/nemo_en_titanet_small.onnx",
                    "https://www.modelscope.cn/models/himyworld/web_core_reference/resolve/master/onnx/3dspeaker_speech_eres2net_large_sv_zh-cn_3dspeaker_16k.onnx"
                ], callback=self._process_callback)
                from web_core_reference.process.prepare_audio import built_speakers as _run_speakers
                del kw['is_cuda']
                ...
                tools.check_and_down_ms(model_id='iic/speech_campplus_speaker-diarization_common',
                                        callback=self._process_callback)
                from web_core_reference.process.prepare_audio import cam_speakers as _run_speakers
            elif speaker_type == 'pyannote':
                from web_core_reference.process.prepare_audio import pyannote_speakers as _run_speakers
            elif speaker_type == 'reverb':
                from web_core_reference.process.prepare_audio import reverb_speakers as _run_speakers
            else:
    ...
    def _split_audio_byraw(self, is_separate=False):
        ...
        if uvr_models.startswith('spleeter'):
            tools.down_file_from_ms(f'{ROOT_DIR}/models/onnx', [
                f"https://www.modelscope.cn/models/himyworld/web_core_reference/resolve/master/onnx/vocals.fp16.onnx",
                f"https://www.modelscope.cn/models/himyworld/web_core_reference/resolve/master/onnx/accompaniment.fp16.onnx"
            ], callback=self._process_callback)

        else:
            tools.down_file_from_ms(f'{ROOT_DIR}/models/onnx', [
                f"https://www.modelscope.cn/models/himyworld/web_core_reference/resolve/master/onnx/{uvr_models}.onnx"
            ], callback=self._process_callback)
        from web_core_reference.process.prepare_audio import vocal_bgm
        # 返回 False None 失败
    ...
    def _get_join_flag(self):
        _join_flag = ""
        if self.cfg.subtitle_type != 3 or not Path(f'{ROOT_DIR}/web_core_reference/ass.json').exists():
            return _join_flag
        try:
            assjson = json.loads(Path(f'{ROOT_DIR}/web_core_reference/ass.json').read_text(encoding='utf-8'))
        except Exception:
