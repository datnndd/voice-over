...
from web_core_reference.configure.excepts import NO_RETRY_EXCEPT, TranslateSrtError, LLMSegmentError, StopTask
from web_core_reference.configure.config import logger, settings, params, ROOT_DIR, tr
from web_core_reference.task.taskcfg import SrtItem
from web_core_reference.translator._base import BaseTrans
...
class OpenAICampat(BaseTrans):
    ...
    def llm_segment(self, srt_list)->List[SrtItem]:
        _st=time.time()
        prompts_template = Path(ROOT_DIR + '/web_core_reference/prompts/recharge/recharge-llm.txt').read_text(encoding='utf-8')
