...
from web_core_reference.configure.config import ROOT_DIR,logger
...
class M2M100Trans(BaseTrans):
    ...
    def _download(self):
        if not Path(f'{ROOT_DIR}/models/m2m100_12b/model.bin').exists():
            tools.down_zip(f"{ROOT_DIR}/models", 'https://modelscope.cn/models/himyworld/web_core_reference/resolve/master/m2m100_12b_model.zip',self._process_callback)
