...
# 数字代表显示顺序
from web_core_reference.configure.config import tr, params, app_cfg, logger, ROOT_DIR
from web_core_reference.translator._base import BaseTrans
from web_core_reference.translator._google import Google
from web_core_reference.util import tools
from web_core_reference import ChannelProvider, get_class
...
# 如果存在新增
try:
    if Path(ROOT_DIR + f'/web_core_reference/newlang.txt').exists():
        _new_lang = Path(ROOT_DIR + f'/web_core_reference/newlang.txt').read_text().strip().split("\n")
        for nl in _new_lang:
            LANGNAME_DICT[nl] = nl
