...
try:
    ...
    class ContentFilterFinishReasonError(Exception): pass
    class InternalServerError(Exception): pass
    class LengthFinishReasonError(Exception): pass
...
from app_core.configure.config import defaulelang


# 内部已整理好错误提示消息的异常，将ex=None,message='{错误消息}'
class app_coreError(Exception):
...
class TranslateSrtError(app_coreError):
    pass


class DubbingSrtError(app_coreError):
    pass


class SpeechToTextError(app_coreError):
    pass


class LLMSegmentError(app_coreError):
    pass

class FFmpegError(app_coreError):
    pass


# 出现该类异常时，需要立即停止任务
class StopTask(app_coreError):
    pass


class StopRetry(app_coreError):
    pass
...
def get_msg_from_except(ex:Exception)->str:
    if isinstance(ex, app_coreError):
        return str(ex)
