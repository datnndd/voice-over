...
from app_core.configure.contants import (
...
def _set_env():
    ...
    os.environ['SOFT_NAME'] = 'pyapp_core'
...
def _get_langjson_list():
    lang_dir = Path(f'{ROOT_DIR}/app_core/language')
    _SUPPORT_LANG={}
    if lang_dir.exists():
...
def _init_language():
    ...
    try:
        _defaulelang = os.environ.get('PYapp_core_LANG', settings.lang)
        if not _defaulelang:
            _defaulelang = QLocale.system().name()[:2].lower() if QLocale else "en"
    except Exception:
        _defaulelang = "en"
...
class AppSettings:
    ...
    _json_path: str = f"{ROOT_DIR}/app_core/cfg.json"
...
class AppParams:
    ...
    """
    _json_path: str = f"{ROOT_DIR}/app_core/params.json"
...
def update_logging_level(new_level_str):
    ...
    _logger = logging.getLogger('app_core')
