...
from app_core.configure.excepts import FFmpegError, app_coreError
from app_core.task.taskcfg import InputFile
from app_core.configure import contants
from app_core.configure.contants import INSTALL_RUBBERBAND_TIPS
...
def get_filepath_from_cmd(cmd: list):
    ...
    for file in file_list:
        if len(file) >= 255:
            return tr(
                'The file path and file name may be too long. Please move the file to a flat and short directory and rename the file to a shorter name, ensuring that the length from the drive letter to the end of the file name does not exceed 255 characters: {},For example D:/app_core/1.mp4 D:/app_core/2.wav',
                file)
        for flag in special:
            if flag in file:
                return tr(
                    'There may be special characters in the file name or path. Please move the file to a simple directory consisting of English and numerical characters, rename the file to a simple name, and try again to avoid errors: {},For example D:/app_core/1.mp4 D:/app_core/2.wav',
                    file)
    return None
...
def get_video_codec(compat=None) -> str:
    ...
    try:
        if not _codec_cache and Path(f'{ROOT_DIR}/app_core/codec.json').exists():
            _codec_cache = json.loads(Path(f'{ROOT_DIR}/app_core/codec.json').read_text(encoding='utf-8-sig'))
    except Exception as e:
    ...
    try:
        test_input_file = Path(ROOT_DIR) / "app_core/styles/no-remove.mp4"
    ...
    if not encoders_to_test:
        ...
        try:
            ...
            # 保存缓存到本地
            Path(f"{ROOT_DIR}/app_core/codec.json").write_text(json.dumps(_codec_cache))
        except Exception as e:
            # 发生异常不缓存
...
def create_concat_txt(filelist, concat_txt=None):
    ...
    if not txt:
        ...
        raise app_coreError("Cannot create concat txt from an empty or invalid file list.")
...
def send_notification(title, message):
    ...
    try:
        notification.notify(
            ...
            ticker="pyapp_core",
            app_name="pyapp_core",
            app_icon=ROOT_DIR + '/app_core/styles/icon.ico',
