...
from app_core.configure.excepts import DubbingSrtError, StopTask
from app_core.configure.config import tr, settings, logger, ROOT_DIR
    ...
    def _cleantts(self) -> None:
        ...
        if settings.get('normal_text'):
            if self.language[:2] == 'zh':
                from app_core.util.cn_tn import TextNorm
                normalizer = TextNorm(to_banjiao=True)
            elif self.language[:2] == 'en':
                from app_core.util.en_tn import EnglishNormalizer
