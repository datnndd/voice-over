...
from app_core.configure.config import tr, app_cfg, logger, ROOT_DIR
from app_core.configure.base import BaseCon
from app_core.configure.excepts import DubbingSrtError, app_coreError
from app_core.task.taskcfg import TaskCfgBase, SrtItem
...
class BaseTask(BaseCon):
    ...
    def _save_srt_target(self, srtstr: List[SrtItem], file: str):
        try:
            ...
            raise app_coreError(f'保存字幕前格式化srt失败:{file=}') from e
