...
from app_core.configure.config import ROOT_DIR, tr, app_cfg, settings, logger
from app_core.configure import config
from app_core.recognition import run as run_recogn, is_allow_lang as recogn_allow_lang
from app_core.translator import run as run_trans, get_audio_code
from app_core.tts import run as run_tts, AZURE_TTS, SUPPORT_CLONE
from app_core.task.simple_runnable_qt import run_in_threadpool
...
class TransCreate(BaseTask):
    ...
    def recogn(self) -> None:
        ...
        if self.cfg.remove_noise:
            ...
            if tools.vail_file(_remove_noise_wav):
                ...
                tools.down_file_from_ms(f'{ROOT_DIR}/models/onnx', urls=[
                    'https://modelscope.cn/models/himyworld/app_core/resolve/master/onnx/dpdfnet4.onnx'],
                                        callback=self._process_callback)
                from app_core.process.prepare_audio import remove_noise
                kw = {
        ...
        if self.cfg.fix_punc and self.cfg.detect_language[:2] in ['zh', 'en']:
            tools.down_file_from_ms(f'{ROOT_DIR}/models/puntc', [
                    "https://www.modelscope.cn/models/himyworld/app_core/resolve/master/puntc/model.onnx",
                    "https://www.modelscope.cn/models/himyworld/app_core/resolve/master/puntc/config.yaml",
                    "https://www.modelscope.cn/models/himyworld/app_core/resolve/master/puntc/tokens.json",
            ], callback=self._process_callback)
            from app_core.process.prepare_audio import fix_punc
            # 预先删掉已有的标点
        ...
        if self.cfg.rephrase==1:
            # LLM重新断句
            try:
                from app_core.translator._openaicompat import OpenAICampat
                ob = OpenAICampat(
    ...
    def recogn2pass(self) -> None:
        ...
        try:
            ...
            if recogn_allow_lang(langcode=self.cfg.target_language_code,
                ...
                logger.warning(f'二次识别配音音频跳过：当前识别渠道不支持目标语言 {self.cfg.target_language_code}')
                return
        ...
        try:
            ...
            if speaker_type == 'built':
                tools.down_file_from_ms(f'{ROOT_DIR}/models/onnx', [
                    "https://www.modelscope.cn/models/himyworld/app_core/resolve/master/onnx/seg_model.onnx",
                    "https://www.modelscope.cn/models/himyworld/app_core/resolve/master/onnx/nemo_en_titanet_small.onnx",
                    "https://www.modelscope.cn/models/himyworld/app_core/resolve/master/onnx/3dspeaker_speech_eres2net_large_sv_zh-cn_3dspeaker_16k.onnx"
                ], callback=self._process_callback)
                from app_core.process.prepare_audio import built_speakers as _run_speakers
                del kw['is_cuda']
                ...
                tools.check_and_down_ms(model_id='iic/speech_campplus_speaker-diarization_common',
                                        callback=self._process_callback)
                from app_core.process.prepare_audio import cam_speakers as _run_speakers
            elif speaker_type == 'pyannote':
                from app_core.process.prepare_audio import pyannote_speakers as _run_speakers
            elif speaker_type == 'reverb':
                from app_core.process.prepare_audio import reverb_speakers as _run_speakers
            else:
    ...
    def align(self) -> None:
        ...
        # 成功后，如果存在 音量，则调节音量
        if self.cfg.tts_type != AZURE_TTS and self.cfg.volume != '+0%' and tools.vail_file(
                self.cfg.target_wav):
    ...
    def _split_audio_byraw(self, is_separate=False):
        ...
        if uvr_models.startswith('spleeter'):
            tools.down_file_from_ms(f'{ROOT_DIR}/models/onnx', [
                ...
                f"https://www.modelscope.cn/models/himyworld/app_core/resolve/master/onnx/accompaniment.fp16.onnx"
            ], callback=self._process_callback)

        else:
            tools.down_file_from_ms(f'{ROOT_DIR}/models/onnx', [
                f"https://www.modelscope.cn/models/himyworld/app_core/resolve/master/onnx/{uvr_models}.onnx"
            ], callback=self._process_callback)
        from app_core.process.prepare_audio import vocal_bgm
        # 返回 False None 失败
    ...
    def _process_subtitles(self) -> Union[tuple[str, str], None]:
        ...
        if not Path(self.cfg.target_sub).exists() :
            raise app_coreError(tr("No valid subtitle file exists")+self.cfg.target_sub)
    ...
    def _get_join_flag(self):
        _join_flag = ""
        if self.cfg.subtitle_type != 3 or not Path(f'{ROOT_DIR}/app_core/ass.json').exists():
            return _join_flag
        try:
            assjson = json.loads(Path(f'{ROOT_DIR}/app_core/ass.json').read_text(encoding='utf-8'))
        except Exception:
    ...
    def _join_video_audio_srt(self) -> None:
        ...
        if not Path(self.cfg.novoice_mp4).exists():
            raise app_coreError(f'{self.cfg.novoice_mp4} 不存在')

        # 需要配音但没有配音文件
        if self.should_dubbing and not tools.vail_file(self.cfg.target_wav):
            raise app_coreError(f"{tr('Dubbing')}{tr('anerror')}:{self.cfg.target_wav}")
        ...
        try:
            ...
            raise app_coreError(tr('Error in embedding the final step of the subtitle dubbing')+str(e)) from e
        finally:
            os.chdir(ROOT_DIR)
        ...
        if Path(tmp_target_mp4).exists():
            try:
                ...
                try:
                    ...
                    raise app_coreError(tr('Translation successful but transfer failed.', tmp_target_mp4)) from e
