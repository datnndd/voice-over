...
from app_core.configure.excepts import NO_RETRY_EXCEPT, TranslateSrtError, LLMSegmentError, StopTask
from app_core.configure.config import logger, settings, params, ROOT_DIR, tr
from app_core.task.taskcfg import SrtItem
from app_core.translator._base import BaseTrans
...
class OpenAICampat(BaseTrans):
    ...
    def llm_segment(self, srt_list)->List[SrtItem]:
        _st=time.time()
        prompts_template = Path(ROOT_DIR + '/app_core/prompts/recharge/recharge-llm.txt').read_text(encoding='utf-8')
