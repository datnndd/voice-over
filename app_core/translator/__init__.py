...
# 数字代表显示顺序
from app_core.configure.config import tr, params, app_cfg, logger, ROOT_DIR
...
# AI翻译渠道，方便判断
AI_TRANS_CHANNELS = [CHATGPT_INDEX, DEEPSEEK_INDEX]
# 翻译通道名字列表，显示在界面
...
# 如果存在新增
try:
    if Path(ROOT_DIR + f'/app_core/newlang.txt').exists():
        _new_lang = Path(ROOT_DIR + f'/app_core/newlang.txt').read_text().strip().split("\n")
        for nl in _new_lang:
            LANGNAME_DICT[nl] = nl
